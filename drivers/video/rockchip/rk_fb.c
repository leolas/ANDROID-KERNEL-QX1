...
static int rk_fb_lcdc_state(int lcdc_id);
//$_rbox_$_modify_$ end
/***************************************************************************
...
static int rk_fb_wait_for_vsync_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		int ret = wait_event_interruptible(dev_drv->vsync_info.wait,
			//!ktime_equal(timestamp, dev_drv->vsync_info.timestamp) &&//Fiv video skip prblem made by phjanderson Big thanks to him
...
//$_rbox_$_modify_$ zhengyang modified for box display system
#if 1
int rk_fb_lcdc_state(int lcdc_id)
{
	struct rk_fb_inf *inf =  platform_get_drvdata(g_fb_pdev);
